    ...
    return (
        <header className={`bg-white fixed top-0 right-0 left-0 z-50 ${isScrolled ? 'drop-shadow-[0_4px_25px_rgba(0,0,0,0.5)]' : ''}`}>
            <nav className='max-w-[1400px] mx-auto px-10 md:h-[14vh] h-[12vh] items-center flex justify-between'>
                {/* logo */}
                <Link to='/' className='text-3xl font-bold'>Gr<span className='text-orange-500 uppercase'>o</span>cery</Link>
                {/* Desktop menu */}
                ...
                <div className='flex items-center gap-x-5'>
                    {/* Mobile menu */}
                    <ul className={`flex items-center gap-x-15 md:hidden absolute top-30 -left-full transform -translate-x-1/2 flex-col gap-y-12 bg-orange-500/15 backdrop-blur-xl rounded-xl p-10 transition-all duration-500 ${showMenu ? 'left-1/2':""}`}>
                    <li>
                        <a href='#' className='font-semibold tracking-wider text-orange-500'>Home</a>
                    </li>
                    <li>
                    ...
                    <li className='flex p-1 border-2 border-orange-500 rounded-full md:hidden'>
                        ...
                        <button className='bg-gradient-to-b from-orange-600 to-orange-300 text-white w-10 h-10 flex justify-center items-center rounded-full text-xl'><IoSearch /></button>
                    </li>
                </ul>
                    {/* input field */}
